...
export function FormatError(input: unknown) {
  if (MCP.Failed.isInstance(input))
    return `MCP server "${input.data.name}" failed. Note, hdp does not support MCP authentication yet.`
  ...
  if (Provider.ModelNotFoundError.isInstance(input)) {
    ...
    return [
      ...
      `Try: \`hdp models\` to list available models`,
      `Or check your config (hdp.json) provider/model names`,
    ].join("\n")
  }
