...
import hdp from "./theme/hdp.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@hdp/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "hdp")
        draft.active = typeof active === "string" ? active : "hdp"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.hdp, store.mode)
    })
