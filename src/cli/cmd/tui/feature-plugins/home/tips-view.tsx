...
const TIPS = [
  ...
  "Press {highlight}Tab{/highlight} to cycle between Analyst, Build and Plan agents",
  ...
  "Use {highlight}/projects{/highlight} or {highlight}Ctrl+P{/highlight} to switch between your local projects",
  "Use {highlight}/new-project{/highlight} or {highlight}pn{/highlight} to add a new project from a directory",
  "Index your files with {highlight}/local_index{/highlight} to enable semantic search",
  "Use {highlight}/local_search{/highlight} to find code snippets by meaning, not just keywords",
  ...
  "HDP auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.hdp/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.hdp/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "HDP auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "HDP uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.hdp/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.hdp/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent HDP from reading sensitive files",
  "Use {highlight}hdp run{/highlight} for non-interactive scripting",
  "Use {highlight}hdp --continue{/highlight} to resume the last session",
  "Use {highlight}hdp run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}hdp serve{/highlight} for headless API access to HDP",
  "Use {highlight}hdp run --attach{/highlight} to connect to a running server",
  "Run {highlight}hdp upgrade{/highlight} to update to the latest version",
  "Run {highlight}hdp auth list{/highlight} to see all configured providers",
  "Run {highlight}hdp agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.hdp/themes/{/highlight} directory",
  ...
  "Run {highlight}hdp debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/hdp{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with HDP Zen for curated, tested models",
