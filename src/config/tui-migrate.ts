...
const TUI_SCHEMA_URL = "https://hdp.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from hdp.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const hdp = await hdpFiles(input)
  for (const file of hdp) {
...
async function hdpFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.HDP_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("hdp", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "hdp")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "hdp"))
  }
  if (Flag.HDP_CONFIG) files.push(Flag.HDP_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "hdp"))
